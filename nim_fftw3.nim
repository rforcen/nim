...
const 
  ...
  FFTW_MEASURE* = 0 # fast plan, slower exec
...
proc fft*(n:int, dir : cint = FFTW_FORWARD, fmethod : cuint = FFTW_MEASURE) : FFT = 
  result = FFT(n:n, cin:newSeq[Complex64](n), cout:newSeq[Complex64](n))
  result.plan = fftw_plan_dft_1d(n, result.cin, result.cout , dir, fmethod)

proc set*(fft:var FFT, x:seq[float])=
  for i in 0..x.high: fft.cin[i]=complex64(x[i],0)

proc power_2*(x:int) : int =
  result = 1
  while result < x:
    result = result.shl 1

proc exec*(fft:var FFT) = fftw_execute(fft.plan)
proc exec*(fft:var FFT, x:seq[float])=
  fft.set(x)
  fft.exec()
