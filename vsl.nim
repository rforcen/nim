...
when isMainModule:
  ...
  proc test_string* =
    const vls_source = """fade=-0.2;"""
    var scanner = newScanner(vls_source)

    echo vls_source
    while true:
      write(stdout, scanner.getsym(), " ")
      if scanner.sym == SNULL: break
    echo ""

  proc test1file* =
    const vls_source = staticRead("test.vsl")
  ...
  import os
  proc test_scanner_folder* =
    const path = "/home/asd/Documents/rust/vsl/samples/*.vsl";
    for file in walkFiles(path):
      ...
      while true:
        ...
        case scanner.sym:
          of SNULL: break
          of IDENT: write(stdout, "=", scanner.get_id(), " ")
          of NUMBER: write(stdout, "=", scanner.get_num(), " ")
          else: discard
      echo "\n", repeat('-', 80)

  proc test_compiler_folder* =
    const path = "/home/asd/Documents/rust/vsl/samples/*.vsl";
    for file in walkFiles(path):
      let source = readFile(file)
      echo "source file: ", file, "\n\n", source, "\n"
      ...
      if compiler.compile():
        var i = 0
        while i < compiler.code.len:
          let c = cast[Symbols](compiler.code[i])
          case c:
            of PUSH_CONST:
              write(stdout, c, " ", cast[float](compiler.code[i+1]), " ")
              inc i
            of POP:
              write(stdout, c, " ", cast[pcode](compiler.code[i+1]), " ")
              inc i
            of PUSH_ID:
              write(stdout, c, " ", compiler.tab_values[cast[pcode](
                  compiler.code[i+1])].id, " ")
              inc i
  ...
  proc test_compiler* =
    let source = """
    const f=409.27,d=10,f1=411, seconds=10;
      ...
      while i < compiler.code.len:
        ...
        case c:
          ...
          of POP:
            ...
            inc i
          of PUSH_ID:
            write(stdout, c, " ", compiler.tab_values[cast[pcode](compiler.code[
                i+1])].id, " ")
            inc i
  ...
  func calc_range(size, i, n: int): Slice[int] =
    let
      chunk_sz = size div n
      rfrom = i * chunk_sz
      rto = if (i+1) * chunk_sz > size: size else: (i+1) * chunk_sz
  ...
  proc test_par* =
    var
      s = 0.float
      n = 50_000_000_000.int
    # for i in 0||n:  s+=i.float
    # echo s,",",(n.float+1)*n.float / 2.0

    s = 0.float
    proc sum(s: var float, size: int, i: int, n: int) =
      for v in calc_range(size, i, n): s+=v.float
