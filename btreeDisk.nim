#[ btree
 ...
 usage:
   vat bt = newBTree(Write, "file.ndx) # create 
...
const
  ...
  NilPtr = -1  
  # set the required size of key & data
  KeyFieldMax = 4 # for int32
  DataFieldMax = 20
...
proc checkSubtree(bt:BTreeDisk, current:int, last: var KeyFieldType)=
  var node: NodeType

  if current == NilPtr: return

  bt.dataFile.setPosition current * bt.nodeSize
  assert bt.dataFile.readData(node.unsafeAddr, bt.nodeSize) == bt.nodeSize
...
proc check*(bt : BTreeDisk)=
  var last : KeyFieldType

  last[0] = '*'
  bt.checkSubtree(bt.root, last)

proc newBtreeDisk*(mode:OpenModes, FileName:string) : BTreeDisk =
...
proc empty*(bt:BTreeDisk):bool = bt.root == NilPtr

proc searchNode(bt:BTreeDisk, target:KeyFieldType , location: var int) : bool =
  var found = false
...
proc split(bt: var BTreeDisk, currentItem:ItemType, currentRight, currentRoot, location: int, 
  ...
  assert bt.dataFile.readData(bt.currentNode.unsafeAddr, bt.nodeSize) == bt.nodeSize
...
proc pushDown(bt:var BTreeDisk,  currentItem:ItemType, currentRoot:int,
...
proc add*(bt:var BTreeDisk, item: ItemType) : bool =
  var
    MoveUp:bool
    NewRight:int
    NewItem:ItemType
...
proc find*(bt:var BTreeDisk,  searchKey:KeyFieldType, item:var ItemType) : bool =
  var
    currentRoot = bt.root
    location = 0
    found = false
...
proc toKey*(i:int32):KeyFieldType=cast[KeyFieldType](i)
...
when isMainModule:
  ...
  proc test_ins_find_random*()=
    var bt = newBtreeDisk(Create, "btree.ndx")

    var rl = (0..n).toSeq() # random list
    rl.shuffle

    echo "inserting random list of ", n, " items..."
    
    for i in 0..n:
      if not bt.add(ItemType(key:toKey(rl[i].int32), data: toData("data" & $rl[i]))): break
    ...
    for i in 0..n:
      var item : ItemType
      if not bt.find(toKey(rl[i].int32), item): 
        ok=false
        break
      else:
        assert item.key == toKey(rl[i].int32)
        assert item.data == toData("data" & $rl[i])
      if i %% (n div 10) == 0: 
  ...
  proc test_ins_find*()=
    ...
    for i in 0..n:
      ...
      if not bt.find(toKey($i), item): 
        ...
        assert item.key == toKey($i)
        assert item.data == toData("data" & $i)
      if i %% (n div 10) == 0: 
        write stdout, i, "\r";   stdout.flushFile
  ...
  proc test_find*()=
    var bt = newBtreeDisk(ReadWrite, "btree.ndx")
  ...
  proc test_random_find*()=
    ...
    for i in 0..niters:
      ...
      if not bt.find(key, item): 
        ...
        assert item.key == key
        assert item.data == toData("data" & r)
      if i %% (n div 10) == 0: 
  ...
  proc test_key_cmp*()=
    assert 10.toKey < 11.toKey
    assert 12.toKey > 10.toKey
    assert 10.toKey >= 10.toKey
    assert 8.toKey <= 68.toKey
    assert 10.toKey == 10.toKey
    assert 10.toKey != 11.toKey
    
    assert "10".toKey < "11".toKey
    assert "7".toKey > "10".toKey
    assert "10".toKey >= "10".toKey
    assert "6".toKey <= "68".toKey
    assert "10".toKey == "10".toKey
    assert "10".toKey != "11".toKey
    echo "key cmp ok!"
