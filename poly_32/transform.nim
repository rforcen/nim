...
proc kisN*(p: Polyhedron; n:int32=0; apexdist:float32=0.1) : Polyhedron =
    let 
    ...
    for nface, face in p.faces:
        let fname : Int4 = i4('f', nface.int32) # [F_TOKEN, nface, 0,0]
        var v1 = face[^1]
        ...
        for v2 in face:
            ...
            if face.len == n or n == 0:
                ...
                flag.add_face(i4(nface.int32), i4(v1), iv2)
...
proc ambo*(p:Polyhedron) : Polyhedron =
    ...
    for i, face in p.faces:
        ...
        for v3 in face:
            ...
            # two new flags:
            flag.add_face(i4(ORIG, i.int32),  m12, m23) # One whose face corresponds to the original f:          
...
proc gyro*(p:Polyhedron) : Polyhedron =
    ...
    for i, face in p.faces:
        ...
        flag.add_vertex(i4(CNTR, i.int32), centers[i])
        ...
        for v3 in face:
            ...
            # 5 new faces
            flag.add_face(@[i4(CNTR, i.int32), i4(v1, v2), i4(v2, v1), i4(v2), i4(v2, v3)])
...
proc propellor*(p:Polyhedron):Polyhedron=
    ...
    for i, face in p.faces:
        ...
        for v3 in face:
            ...
            flag.add_face(i4(i.int32), i4(v1, v2), i4(v2, v3)) 
...
proc dual*(p:Polyhedron):Polyhedron=
    ...
    for i, face in p.faces:
        flag.add_vertex(i4(i.int32), centers[i], with_unit=false)

        var v1 = face[^1]
        for v2 in face:
            if face.len>2 and i4(v2, v1) in face_map:
                flag.add_face(i4(v1), i4(face_map[i4(v2, v1)]), i4(i.int32));
                v1 = v2
            else: ok=false
...
proc chamfer*(p:Polyhedron, dist : float32 = 0.5):Polyhedron=
    if not p.check: return p
...
proc whirl*(p:Polyhedron):Polyhedron=
    ...
    for i, face in p.faces:
        ...
        for v3 in face:
            ...
            let 
                cv1name = i4('c', i.int32, v1)
                cv2name = i4('c', i.int32, v2)
            ...
            flag.add_face(i4('c', i.int32), cv1name, cv2name)
...
proc quinto*(p:Polyhedron):Polyhedron=
    ...
    for nface, face in p.faces:
        ...
        for v3 in face:
            let 
                t12 = i4_min(v1, v2)
                ti12 = i4_min(nface.int32, v1, v2)
                t23 = i4_min(v2, v3)
                ti23 = i4_min(nface.int32, v2, v3)
                iv2 = i4(v2)
...
proc insetN*(p:Polyhedron, n:int32=0, inset_dist:float32=0.3, popout_dist:float32 = -0.1):Polyhedron=
    if not p.check: return p
    ...
    for i, face in p.faces:
        ...
        for v2 in face:
            if face.len == n or n==0:
                foundAny=true
                flag.add_vertex(
                    i4('f'.toToken, i.int32, v2),
                    tween(p.vertex[v2], centers[i], inset_dist) + ( normals[i] * popout_dist),
                    with_unit = false
                )

                flag.add_face(@[i4(v1), i4(v2), i4('f', i.int32, v2), i4('f', i.int32, v1)])
                # new inset, extruded face
                flag.add_face(i4('e', i.int32), i4('f', i.int32, v1), i4('f', i.int32, v2))
            else:
                flag.add_face(i4(i.int32), i4(v1), i4(v2)) # same old flag, if non-n
...
proc extrudeN*(p:Polyhedron, n:int32=0):Polyhedron=
    var poly = p.insetN(n=n, inset_dist=0.0, popout_dist=0.3)
    poly.name="x" & (if n!=0: $n else: "") & p.name
    poly

proc loft*(p:Polyhedron, n:int32 = 0, alpha : float32 = 0.1):Polyhedron=
    var poly = p.insetN(n=n, inset_dist=alpha, popout_dist=0.0)
    poly.name="l" & (if n!=0: $n else: "") & p.name
    poly

proc hollow*(p:Polyhedron, inset_dist : float32 = 0.3, thickness : float32 = 0.1):Polyhedron=
    if not p.check: return p
...
proc trisub*(p:Polyhedron, n:int32=2):Polyhedron=
    if p.faces.mapIt(it.len != 3).any(proc (x:bool):bool=x): return p
        ...
        for fn, face in p.faces:
            ...
            for i in 0..n:
                for j in 0..n-i:
                    let v = (v1 + v21 * (i.float32 * 1.0 / n.float32)) + (v31 * (j.float32 * 1.0 / n.float32))
                    vmap["v" & $fn & "-" & $i & "-" & $j] = pos
                    inc pos
                    newVs.add(v);
        ...
        result=Polyhedron(name : "u" & $n, vertex: uniqVs, faces: faces)
        result.simplify

when isMainModule:
    import times

    echo "generating: qqqqqD"
    var p = Dodecahedron
    p=p.quinto.quinto.quinto.quinto.quinto
    p.set_normals
    
    echo "#faces:", p.faces.len

    for i in 0..3:
        var t0=now()
        discard p.calc_areas
        let ts=(now()-t0).inMilliseconds
        echo ts