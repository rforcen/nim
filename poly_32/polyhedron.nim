    ...
    proc draw(p:Polyhedron) =
        ...
        proc keyhook(key:GLbyte, x,y:GLint) {.cdecl.} =
            ...
            proc gen_poly(tr:string)=
                let t0=now()
                gpoly = transform(tr, limit_vertex=10000)
                let lap=(now()-t0).inMilliseconds()

                echo $gpoly & fmt(", lap:{lap}ms")
...
proc main=
    ...
    let t0=now()
    var tr = if paramCount()==1: paramStr(1) else: "aqaqaqD" #rand_preset()
    var p = transform(tr, limit_vertex=100000) 
    # p.write_wrl
    let lap=(now()-t0).inMilliseconds()
    echo $p & fmt(", lap:{lap}ms")
    draw(p)
